...
public class ApiServiceAction extends AbstractApiAction {
    ...
    /**
     * Copy an exist widget (physic and with parameters, joined with a exist api method) and value the form of creation
     * of new api service.
     *
     * @return The result code.
     */
    public String copyFromWidget() {
        try {
            ...
            WidgetType masterWidgetType = this.getWidgetTypeManager().getWidgetType(masterWidget.getTypeCode());
            WidgetType type = (masterWidgetType.isLogic()) ? masterWidgetType.getParentType() : masterWidgetType;
            if (null == masterMethod.getRelatedWidget()
                ...
                this.addFieldError("framePos", this.getText("error.service.paste.invalidWidget",
                        new String[]{masterWidget.getTypeCode(), masterMethod.getResourceName()}));
                return INPUT;
            }
    ...
    private ApsProperties extractParametersFromWidget(ApiMethodRelatedWidget relatedWidget, Widget masterWidget) {
        WidgetType masterWidgetType = this.getWidgetTypeManager().getWidgetType(masterWidget.getTypeCode());
        ApsProperties showletProperties = (masterWidgetType.isLogic()) ? masterWidgetType.getConfig() : masterWidget.getConfig();
