...
class CacheInfoManagerTest {
    ...
    void updateFromPageChanged() {
        ...
        Mockito.verify(cache, Mockito.times(0)).put(Mockito.anyString(), Mockito.any());
    }
    ...
    private void flushGroup(String groupName, int expectedEvict) {
        ...
        Mockito.verify(cache, Mockito.times(expectedEvict + 1)).evict(Mockito.any(Object.class));
        Mockito.verify(cache, Mockito.times(0)).put(Mockito.any(), Mockito.any(Object.class));
    }

    private void initGroups(String targetCache) {
        List<String> list_a = new ArrayList<>(List.of("key_a1", "key_a2", "key_a3"));
        List<String> list_b = new ArrayList<>(List.of("key_b1", "key_b2", "key_b3", "key_b4"));
        String key1 = "CacheInfoManager_groups_" + targetCache + "__GR__group_1";
        String key2 = "CacheInfoManager_groups_" + targetCache + "__GR__group_2";
        Cache.ValueWrapper valueWrapperForGroups1 = Mockito.mock(Cache.ValueWrapper.class);
        Cache.ValueWrapper valueWrapperForGroups2 = Mockito.mock(Cache.ValueWrapper.class);
        Mockito.lenient().when(cache.get(key1)).thenReturn(valueWrapperForGroups1);
        Mockito.lenient().when(cache.get(key2)).thenReturn(valueWrapperForGroups2);
        Mockito.lenient().when(valueWrapperForGroups1.get()).thenReturn(list_a);
        Mockito.lenient().when(valueWrapperForGroups2.get()).thenReturn(list_b);
    }
