						...
						<c:if test="${occurrences[facetNode.code] != null}">
							<li>
								<a href="<wp:url><wp:parameter name="selectedNode" ><c:out value="${facetNode.code}" /></wp:parameter>
									...
									</wp:url>"><wpfp:facetNodeTitle facetNodeCode="${facetNode.code}" /></a>&#32;<abbr class="jpsolr_tree_occurences" title="<wp:i18n key="jpsolr_OCCURRENCES_FOR" />&#32;<wpfp:facetNodeTitle facetNodeCode="${facetNode.code}" />:&#32;<c:out value="${occurrences[facetNode.code]}" />"><c:out value="${occurrences[facetNode.code]}" /></abbr>
